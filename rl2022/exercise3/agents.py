    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        self.epsilon = self.eps_end + (self.eps_start - self.eps_end) * np.exp(-1. * (timestep/100) / self.eps_decay)
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # follow the e-greedy policy if explore flag True
        if explore:
            if np.random.random() < self.epsilon:
                return self.action_space.sample()

        # otherwise follow the greedy policy
        return torch.argmax(self.critics_net(torch.tensor(obs))).numpy()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # get the state action values from the main network
        predicted_Q = self.critics_net(states).gather(1, actions.type(torch.int64))
        # get the best Q-values from the target network returns for each state in batch
        target_Q = torch.max(self.critics_target(next_states), 1)[0].detach()
        expected = (1 - done.squeeze(1)) * target_Q * self.gamma + rewards.squeeze(1)

        # calculate loss
        loss = torch.nn.MSELoss()(predicted_Q.squeeze(1), expected)
        ...
        # check the status of the update counter and copy weights from main to target network
    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # get a list of probabilities for the actions given the observation
        probs = self.policy(torch.tensor(obs))
        # build distribution given the probs
        dist = torch.distributions.Categorical(probs=probs)
        # sample this distribution to get the action
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        returns = []
        # loop over rewards in episode and compute returns at each timestep
        ...
        loss = 0
        # iterate over episode and update policy at each step
        for state, action, G in zip(observations, actions, returns):
            # get our distribution
            probs = self.policy(torch.tensor(state))
            dist = torch.distributions.Categorical(probs=probs)

            # use the distribution to evaluate the action
            log_prob = dist.log_prob(torch.tensor(action))
            # calculate loss
            loss = loss - G * log_prob
