    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        # initialise all values for each state to zero
        ...
        k = 0
        # flag to break out of value iteration loop
        converged = False

        # outermost loop for each iteration k
        while not converged:
            delta = 0

            for state in self.mdp.states:
                # keep track of the old state value from previous iteration
                oldVal = V[self.mdp._state_dict[state]]
                # keep track of returns we get for taking each action
                returns = np.zeros([self.action_dim])

                for action in self.mdp.actions:
                    # loop through all next states
                    for nextState in self.mdp.states:
                            # perform bellman update rule
                            returns[self.mdp._action_dict[action]] += (self.mdp.P[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]]
                                           *(self.mdp.R[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]]
                                             +self.gamma*V[self.mdp._state_dict[nextState]]))

                # set Value to the max return from all actions
                V[self.mdp._state_dict[state]] = np.max(returns)

                # get max from delta and (diff between old V and new V)
                delta = max(delta, np.abs(oldVal-V[self.mdp._state_dict[state]]))
            # check if converged and break loop if true
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for state in self.mdp.states:
            ...
            # get best action from the returns values
            best_action = np.argmax(returns)
            # update policy
            policy[self.mdp._state_dict[state], best_action] = 1
    ...
    def _policy_eval(self, policy: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while not converged:
            delta = 0
            for state in self.mdp.states:
                # to hold the total value for taking that action
                value = 0

                # for possible next actions
                for action in self.mdp.actions:
                    # for possible next states
                    for nextState in self.mdp.states:
                        # perform bellman update rule
                        value += policy[self.mdp._state_dict[state], self.mdp._action_dict[action]] * (self.mdp.P[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]] \
                                 * (self.mdp.R[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]]
                                    + self.gamma * V[self.mdp._state_dict[nextState]]))

                # get max from delta and (diff between value and new V)
                delta = max(delta, np.abs(V[self.mdp._state_dict[state]] - value))
                # set Value to the max return from all actions
                V[self.mdp._state_dict[state]] = value
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for state in self.mdp.states:
                # get the action to take determined by the current policy
                current_action = np.argmax(policy[self.mdp._state_dict[state]])
                # keep track of returns we get in each state for taking the action defined by our policycls
                returns = np.zeros([self.action_dim])

                for action in self.mdp.actions:
                    for nextState in self.mdp.states:
                        # perform bellman update rule
                        returns[self.mdp._action_dict[action]] += self.mdp.P[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]] * (self.mdp.R[self.mdp._state_dict[state], self.mdp._action_dict[action], self.mdp._state_dict[nextState]]
                                    + self.gamma * V[self.mdp._state_dict[nextState]])
                ...
                # update policy
                policy[self.mdp._state_dict[state]] = np.eye(self.action_dim)[best_action]
